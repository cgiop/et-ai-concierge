...
def _heuristic_persona(text: str) -> PersonaProfile:
    t = text.lower()
    low = sum(1 for w in ("safe", "fd", "low risk", "first time", "new to") if w in t)
    high = sum(1 for w in ("trading", "fno", "crypto", "aggressive", "high risk") if w in t)
    loan = sum(1 for w in ("loan", "emi", "home loan", "credit") if w in t)
    if loan >= 1:
        ptype = PersonaType.LOAN_SEEKER
    elif high > low:
        ptype = PersonaType.ACTIVE_TRADER
    elif "sip" in t or "long" in t:
